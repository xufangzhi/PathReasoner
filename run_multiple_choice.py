...
with open('./graph_building_blocks/relation_words_set.json', 'r') as f:
    relations = json.load(f)  # key: relations, value: ignore
...
def main():
    ...
    #     if name.startswith("roberta.encoder.layer.23") or name.startswith("roberta.encoder.layer.22"):
    #         param.requires_grad = True
    ...
    optimizer_grouped_parameters = [
        ...
        {
            "params": [p for n, p in model.named_parameters() if n.startswith("_opt")
                        and not any(nd in n for nd in no_decay)],
            "lr": model_args.roberta_lr,
            "weight_decay": training_args.weight_decay,
        },
    ...
    if training_args.do_eval:
        ...
        output_test_file = os.path.join(training_args.output_dir, "eval_predictions_prob.npy")
        np.save(output_test_file, preds)
